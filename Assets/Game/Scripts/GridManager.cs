    ...
    private void CreateGrid()
    {
        for (int i = 0; i < _gridSize; i++)
        {
            for (int j = 0; j < _gridSize; j++)
            {
                ...
                Tiles[i, j].indisX =
                    i; //Enter the index information into the ActiveX script that holds the information of each tile.
                Tiles[i, j].indisY = j;
        ...
        transform.position =
            new Vector2(GridW / 2 + tileSize / 2, GridW / 2 + tileSize / 2); //set the position each tile parent object
    }
